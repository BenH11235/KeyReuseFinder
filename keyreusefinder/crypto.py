...
RandomLowercase = uniform(string.ascii_lowercase)
...
English1GramsUpper = Distribution({item.upper():prob for item,prob in English1Grams.probs.items()})
...
def chrxor(chr1, chr2):
    #kludge to get python2 legacy code to play nice with python3
    if type(chr1)==str:
        chr1 = ord(chr1)
    if type(chr2)==str:
        chr2 = ord(chr2)
