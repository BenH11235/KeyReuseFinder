...
def xtTable(buf,threshold):
    ...
    xorText = crypto.distributionFromFunction(\
            [crypto.softDist]*2, lambda t: crypto.chrxor(t[0],t[1])\
    )
    return [[\
...
def findparallelciphers(buf):
    ...
    for diagonal in diagonals(len(buf)):
        ...
        values = [table[c[0]][c[1]] for c in diagonal] 
...
def encryptedBySameKeyImplausibility(s1, s2, ptDist):
    ...
    xorText = crypto.distributionFromFunction(\
            [ptDist]*2, lambda t: crypto.chrxor(t[0],t[1])\
    ) 
...
def find_suspected_reuses(data):
    ptDistribution = crypto.softDist
    threshold = (2*log(len(data),2))-1
        
    ptxts =  partition(ptVector(data), (threshold+1)/2, (threshold+1)/2)
    suspectedReuses = findparallelciphers(data)
    return suspectedReuses
...
if __name__ == "__main__":
    ...
    suspectedResuses = find_suspected_reuses(inputBuffer)
